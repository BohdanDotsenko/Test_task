function attempt(available, allowed, preferred) {
    ...
    if (!allowed.includes('any')){    
        for (let i = 0; i < available.length; i++) {    //fill the array valid(allowed and available numbers)
        ...
        valid = available;          
    if (preferred.includes('any')){ // if 'any' element is found in preferred than all valid numbers are preferred
        return valid;
    }
    else {
        let notPreferrad = []    // all not preferred but valid numners
        for( let i = 0; i < preferred.length; i++) { // push all preferred and valid numbers
            ...
            let buffer = Infinity;
            if(notPreferrad.length == 1) {     // if we have one not preferred number find close in value or min valid number
            ...
            else if(notPreferrad.length > 1) { // if we have more than one not preferred numbers we will find max not preferred valid number
                for(let i = 0; i < valid.length; i++) {
                    if (!preferred.includes(valid[i]))
                        buffer = valid[i];
                }
